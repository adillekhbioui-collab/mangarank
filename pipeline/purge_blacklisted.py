...
# Load env variables (can be in parent or backend dir)
        ...
        with open(csv_path, "r", encoding="utf-8-sig") as f:
            ...
            for row in reader:
                ...
                if genre and flag in {"yes", "true", "1", "y"}:
                    blacklisted.add(genre)
    except Exception as e:
        print(f"Error reading {csv_path}: {e}")
        sys.exit(1)
    return blacklisted

def fetch_all_manga(table="manga") -> list[dict]:
    ...
    while True:
        ...
        if r.status_code not in (200, 206):
            # If the table doesn't exist or fails, just break and return what we have
            print(f"  ⚠ Fetch failed at offset {offset}: {r.status_code} - {r.text[:100]}")
            break
...
def delete_manga_batch(table: str, ids_to_delete: list):
    headers = supabase_headers()
    chunk_size = 40  # Keep chunks small to avoid URL length limits
    total_deleted = 0

    log_filename = f"purge_log_{table}.txt"
    with open(log_filename, "w", encoding="utf-8") as log:
        for i in range(0, len(ids_to_delete), chunk_size):
            ...
            if r.status_code in (200, 204):
                ...
                print(f"  ✓ Deleted batch {i // chunk_size + 1} ({len(chunk)} records) from {table}")
                for cid in chunk:
...
def process_table(table: str, blacklisted_genres: set):
    ...
    for rec in records:
        ...
        # Supabase sometimes returns arrays as strings
        ...
        if isinstance(genres, list):
            ...
            if has_blacklisted:
                ...
                # print(f"  Found NSFW: {title} (ID: {rec_id})")

    print(f"  Identified {len(ids_to_delete)} manga containing blacklisted genres out of {len(records)}.")

    if ids_to_delete:
        print(f"  Executing permanent DELETE on {table}...")
        deleted_count = delete_manga_batch(table, ids_to_delete)
        print(f"  Success: Deleted {deleted_count} rows in {table}.")
    else:
...
if __name__ == "__main__":
    ...
    print(f"Loaded {len(blacklisted_genres)} blacklisted genres:")
    print(f"  {', '.join(blacklisted_genres)}")

    # Target the raw table and the final synced table
    process_table("manga_raw", blacklisted_genres)
    process_table("manga_rankings", blacklisted_genres)

    print("\n✅ Purge complete! Run your scrapers again to verify they act as a firewall.")
